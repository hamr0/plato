...
function renderAbout(req, res, { db, auth }) {
  ...
  const howItWorks = html`<section class="about-section">
    ...
    <p><strong>the operator.</strong> the person hosting this instance runs the server but has no special power inside the forum — no admin role, no override of sub mods, no quiet removals. their job is hosting; community decisions stay with mods and members.</p>
  ...
  const rows = subs.length === 0
    ...
    : html`<table class="communities">
        ...
        <tbody>${subs.map((s) => html`<tr>
          <td>${modSet.has(s.name) ? html`<span class="mod-indicator" title="you mod this sub">&gt;</span>` : html``}<a class="sub-link sub-${subColorIndex(s.name)}" href="/sub/${s.name}">//${s.name}</a>${importedSubChip({ sub: s })}${s.sensitive ? html` <span class="sensitive-mark" title="sensitive content — use discretion">[!]</span>` : html``}${s.disabled_at != null ? html` <span class="muted" title="this sub is read-only — awaiting reactivation or community-fork">[read-only]</span>` : html``}${subscribedSet && subscribedSet.has(s.name) ? html` <span class="subscribed-mark" title="you're subscribed to this sub">[in]</span>` : html``}</td>
...
function renderMemlog(req, res, { db, auth }, searchParams) {
  ...
  // the notification half; activity-side slugs (my-posts/my-comments)
  // narrow the activity half. The two are independent — selecting one
  // family hides the other in `all` mode, mirroring the user's
  // expressed intent.
  //
  // Auto-flip: in notifications mode, if a `show: unread` query
  // returns nothing but `show: all` would have rows, flip to `all`
  // and surface a hint. Saves the user from chasing why a filter
  // "didn't work" when the matching rows were just already-read.
  let rows = [];
  let autoFlipped = false;
  if (filters.mode === 'notifications') {
    rows = listNotifications(db, handle, { show: filters.show, kinds: notifKinds, limit: 200 });
    if (rows.length === 0 && filters.show === 'unread') {
      const allRows = listNotifications(db, handle, { show: 'all', kinds: notifKinds, limit: 200 });
