...
- **B1 — sticky note per sub.** Exactly one mod-editable text field per sub, rendered above the feed on `/sub/<name>`. Markdown allowed (subset: bold, italic, links — no images, same renderer escape posture as posts). Max 200 chars (one short paragraph; longer belongs in a pinned post, but pinning is permanently out — see PRD §Permanently out). Editable from `/sub/<name>/edit` only by mods (owner or co). New column `subs.sticky_note TEXT` (migration 022). No history; edits overwrite. **Locks**: one note per sub, mods only, 200-char max, no algorithmic feed promotion of any post (sticky note is the *only* mod-voice slot above the feed). Post-pinning stays permanently out.
