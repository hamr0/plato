...
- **B4 — health watcher cron.** `bin/health-watch.sh`, ~30 LOC. curls `/healthz`, on non-2xx writes a structured one-line stamp to `BACKUP_DIR/health.log` and (if `HEALTH_ALERT_EMAIL` env is set) emails the operator via `mail` / `sendmail` (whichever is on PATH). Email body: failure type, last 30 lines of stderr/stdout from the plato process if reachable, the 5 most recent rows of `health.log`, and a copy-paste GitHub issue template (project URL, plato version, reproduction note, the diagnostic block). The intent: when something breaks at 3am, the operator wakes up with a ready-to-paste issue.
