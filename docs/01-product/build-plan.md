...
- **B3 — backup script.** `bin/backup.sh`, local-only by default. Atomic `sqlite3 forum.db ".backup target.db"` (online, lock-safe) + `tar` of `posts/`, `exports/`, `config.json`, `spam-patterns.txt`, `data/urlhaus.txt`, `disposable-domains.txt`. Output: `BACKUP_DIR/plato-backup-<YYYY-MM-DD-HHMMSS>.tar.gz`. Rotates last N (env `BACKUP_KEEP`, default 7). Operator-guide section with sample crontab + restore procedure (stop → unpack → cp .db + posts/ → start). Off-host copy is a commented one-line rsync stanza in the script — operator opts in by uncommenting; we don't bake SSH key setup into plato.
