...
- **B2 — `/healthz` endpoint.** Public route returning `{ok, version, uptime_s, db_writable, exports_dir_writable, last_migration}`. ~15 LOC, no auth, no body parsing, `Cache-Control: no-store`. Exits non-2xx if any check fails (DB not openable, exports dir unwritable). Under "five runtime deps" — same `node:http` handler as everything else.
