...
**Post-M8 — co-tenant deploy mode for `bootstrap.sh` — BACKLOG.** `deploy/bootstrap.sh` is all-or-nothing: it writes its own nginx server block and runs the deploy-guide mail step (sets `inet_interfaces=loopback-only`, rewrites `mydestination`/`virtual_transport`). On a box already serving another app this is destructive — it clobbers a hand-written co-tenant nginx vhost and severs the neighbor's inbound mail. Proven during the terribic→gitdone consolidation (2026-05-24): bootstrap had to be skipped entirely and nginx + cert staging + opendkim (additive-only) + cron + logrotate hand-rolled, because gitdone runs nginx + inbound postfix on the same host. Proposed shape: `--skip-nginx` (operator brings their own vhost + cert) and `--skip-mail` / additive-mail mode (plato only adds its own opendkim signing key + SigningTable line for its domain, never touches `inet_interfaces`/`mydestination`). The genuinely shared steps (package install, `plato` user, `/opt/plato` clone, cron, logrotate) stay safe to run. Fits the fork-friendly / single-binary / hobby-scale ethos: people will run plato next to other apps on one cheap VPS. Not a v1 blocker; a quality-of-life fix for multi-app operators.
