...
- **B0 — light/dark theme toggle.** Two-state button (no OS-default once clicked). Operator-overridable palettes via `branding.colors` (existing) + new `branding.colorsLight`. `data-theme` attribute on `<html>`, `localStorage.theme` persistence, anti-flash inline `<script>` in head. Button position: last item in header right-cluster. Implementation: extend `style.css` with `[data-theme="light"]` + `@media (prefers-color-scheme: light)` blocks; new ~30-LOC `static/theme.js`; `app.js` extends `resolveBrandingColors` to take a second palette; `bin/server.js` passes `branding.colorsLight` through.
