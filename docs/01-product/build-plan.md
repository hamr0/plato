...
| M6 | Subscriptions + notifications | Sub subscribe, my-subs, three-tier RSS (per-sub public, all-subs token-gated, all-subs+notifications token-gated) | ~2 wks |
...
- **Flairs (per-sub) — SHIPPED M5/B10.** Migration 009 added `subs.flairs` JSON column, `subs.flairs_required`, and nullable `posts.flair_slug`/`drafts.flair_slug`. Owner curates the closed list (max 6 per sub, `MAX_FLAIRS_PER_SUB`) at `/sub/create` or via owner-only `/sub/<name>/edit`; users pick from it at post time. `flairsRequired` enforced in `finalizeDraft`. Display: colored pill in `authorMeta`, filter strip at top of sub page, filter via `?flair=<slug>`. No global flair index, no user-created flairs, no cross-sub flairs. Color is 6-digit hex (`^#[0-9a-f]{6}$`) — what the editor's 8 preset swatches and free-form `<input type="color">` emit; rgb()/named/CSS-keyword forms hard-rejected at validate time (no inline-style XSS surface).
...
Sub subscribe/unsubscribe, my-subs page, three-tier RSS (per-sub public, all-subs token-gated, all-subs+notifications token-gated). Subscription list export (folds into M7's export format).

**Push and email digest both intentionally not shipped.** ntfy was on the original plan but cut: self-hosted ntfy on iOS can't deliver real push (Apple's APNs gate routes only via `ntfy.sh`), so the experience would silently work on Android and feel broken on iOS. Email digest was also on the plan but cut: would have required either plato persisting plaintext addresses (breaks the auth-layer "never stored" lock) or coupling to a knowless email-retention feature, plus scheduler / cadence config / opt-out tokens / footer rendering / bounce handling / operator deliverability burden — for a forum that explicitly rejects urgency engineering. Pull-shape RSS (three tiers) covers the same ground honestly. Both are now PRD-locked under §Permanently out.
...
- **Per-sub RSS — SHIPPED M6/B4.** Atom 1.0 feed at `/sub/<name>/rss`. Latest 50 posts in newest-first order, **excluding both hard-removed and soft-collapsed** (RSS bridges plato to readers in feed shape, not in drama shape — feed readers have no "this is collapsed" affordance, so soft-collapsed entries would land too loud). Title + author pseudonym (not raw handle) + body excerpt (≤600 chars, same `getPostPreview` shape as the modlog target preview). Headers: `Content-Type: application/atom+xml; charset=utf-8`, `Cache-Control: public, max-age=300`. The sub HTML page advertises the feed via `<link rel="alternate" type="application/atom+xml">` (autodiscovery for reader extensions) plus a visible `rss` link in the action row. 404s for unknown subs. Plato's role ends at "publish a feed per sub" — cross-source aggregation belongs to whatever reader the user picks (PRD §Per-sub RSS).
- **Subscribe inline on `/subs` directory — SHIPPED M6/B5.** New per-row column rendering an inline `subscribe`/`unsubscribe` text-link button (logged-in only; column header + cell hidden for anonymous). Reuses the same POST `/sub/<name>/subscribe` endpoint and the same `.subscribe-form`/`.subscribe-btn` styling as the sub-page header button. The directory becomes a one-screen subscription manager — no need to click into each sub to follow it.

- **Personal aggregated RSS — SHIPPED M6/B6.** Two token-gated routes sharing one per-user token: `/u/<token>/subs.rss` (latest 50 posts merged across the user's subscribed subs, drama-shape exclusion of removed + collapsed) and `/u/<token>/rss` (the above plus memlog notifications interleaved by time). Migration 015 adds nullable `handles.rss_token TEXT` with a UNIQUE partial index. Token is 32 random bytes hex-encoded (64 chars), generated lazily on first `/memlog` visit, regenerable via POST `/memlog/rss-regenerate` from a button on the memlog page. Rotation invalidates both URLs at once. No handle in URL — token *is* the credential, keeps pseudonym out of reader logs / proxies / screenshots. Headers: `Content-Type: application/atom+xml`, `Cache-Control: private, no-store`. 404 on bad/missing token (hex shape validated before any DB hit). Reuses the per-sub Atom builder; subs feed = `listPostsAcrossSubs({ subNames: subscribedSet })`; combined feed adds memlog rows. Disallowed in `robots.txt` (`Disallow: /u/`). New module `src/content/rss-token.js` (`getOrCreateRssToken / regenerateRssToken / handleByRssToken`). PRD §Outbound channels.
...
- **B2-b — HTTP routes, sub + user UX, memlog wiring — SHIPPED.** Adds the eligibility gate (mod or 60d continuous subscriber for sub-export; logged-in for personal-export), per-kind production SLA (sub 7d, user 3d) + worker pre-tick `markStaleAsFailed` sweep, per-kind download TTL (3d both kinds, picked for disk-pressure not policy). Routes: `POST /sub/<name>/export-request` (idempotent, gated, 302 → /memlog?export=queued), `POST /export-request` (personal, no gate beyond auth), `GET /export/<token>.tar.gz` (token-bearer, no auth check — same posture as `/u/<token>/rss`). Sub-page action pill renders 5 visual states (anon/non-sub/<60d/eligible/active-job); personal-export pill on `/memlog`. New `src/archive/user-export.js` builds the per-user archive: cross-sub posts + comments, votes-cast (the user's own only — never other voters' handles), subscriptions, subs moderated, mod actions taken/received. Manifest's `scope.handle_attribution` carries the public pseudonym, never the secret handle. Memlog notification kinds `export_ready` (links through bearer token) and `export_failed` (carries reason in snippet). +56 tests across the chain (623 → 679).
- **B3 — per-user export.** Folded into B2-b's `src/archive/user-export.js`. Standalone item complete.
- **B4 — Ed25519 archive signing — SHIPPED.** Migration 019 adds a single-row `instance_keypair` table; `src/archive/signing.js` exposes `getOrCreateInstanceKeypair`, `signBytes`, `verifyBytes`, `fingerprintFromPublicKey`. Worker signs the gzipped tarball on completion and writes a sibling `<archive>.tar.gz.sig` (raw 64-byte Ed25519). Builders thread `pubkeyFingerprint` into the manifest. New routes: `GET /.well-known/plato-pubkey` (JSON-shaped pubkey advertisement), `GET /export/<token>.tar.gz.sig` (token-bearer detached-sig download), `/about` surfaces the fingerprint with verification recipe. Privkey lives in the DB (respects "one SQLite file" rule); lazy-generated on first need; never rotated in v1. Spec moved out of `archive-format.md`'s "Future layers" into a Signing section. +21 tests (679 → 700).
- **B5 — sub-import — SHIPPED.** URL-fetch import (no uploads, per the no-uploads architectural lock). Migration 020 adds `import_jobs` table mirroring `export_jobs` (pending/in-progress/succeeded/failed via timestamps + retry_count; idempotence index on `(source_scope_sub, source_exported_at)` so the same source archive can only succeed once). New columns: `subs.imported_from_url|fingerprint|at|at_source` for the imported badge; `handles.imported_from_fingerprint` for synthetic non-claimable rows; `mod_actions.imported_from_fingerprint` for the `[imported]` modlog tag. New modules: `src/archive/extract.js` (POSIX USTAR reader, mirror of `tar.js`), `src/archive/import.js` (pure DB inserts under fresh handles, with `pseudonymForImport` bracket-wrap on collision), `src/archive/import-queue.js` (state machine + SLA sweep). New worker `bin/run-import-queue.js` (off-peak gating, env `IMPORT_OFFPEAK_*`; size cap `IMPORT_MAX_BYTES` default 500MB; 120s fetch timeout). New routes: `POST /sub/import` (auth-only, URL/rename validation, redirects to `/memlog?import=queued`), `GET /sub/create?mode=import` (second tab on the create page). New memlog notification kinds `import_ready` / `import_failed` plus a new `imports` filter chip. `[imported]` tag prefix on every imported mod action across all three modlog views. Imported-sub banner on `/sub/<name>` showing source host + import date + importer pseudonym. +33 tests (700 → 733).
- **B6 — OpenTimestamps anchor — SHIPPED.** Operator-opt-in via the official `ots` CLI (`apt install opentimestamps-client` or pipx); zero new npm deps (kept the "five runtime deps" posture). Pattern lifted from gitdone (`~/PycharmProjects/gitdone/app/src/ots.js` and `bin/ots-upgrade.js`). `src/archive/timestamp.js` exposes `stampFile` + `upgradeFile`, both ENOENT-tolerant — if the binary is missing, they return `{ error: 'ots not found' }` and exports proceed normally. Export worker calls `stampFile` after writing the .sig (best-effort polish, never blocks the export). New cron `bin/run-ots-upgrade.js` walks `EXPORTS_DIR/*.tar.gz.ots` and runs `ots upgrade` in place; the bytes-changed-after-upgrade signal indicates the proof got Bitcoin-anchored. New token-bearer route `GET /export/<token>.tar.gz.ots` mirrors the .sig route; 404 explicitly says "operator may not have opted in" so importers can distinguish "not stamped" from "stamp failed." `/about` documents the verification recipe with a link to opentimestamps.org. +11 tests (timestamp wrapper unit + .ots route end-to-end). 757/757 green.
- **Followup — sub-archive export surfaces in public modlog — SHIPPED.** Sub-exports are public-facing transparency events; the community deserves to know who has taken a copy. Migration 021 adds `'export'` to the `mod_actions` action CHECK enum. New `recordSubExport(db, ...)` helper in `src/content/mod.js` bypasses the mod-role gate (export eligibility is mod OR 60d-tenured subscriber, not mod-only). `completeJob` writes one row per completed sub-export — parent + each sentinel sibling fan-out + same-day shared-artifact dedupe paths each credit their own requester. Personal exports (`kind='user'`) explicitly do NOT write a modlog row (private). Failed exports do not write a row. `MOD_ACTION_LABELS.export = 'archive exported'` in the renderer. Imported sub-archives carry historical export rows verbatim with the existing `[imported]` tag pattern (M7/B5). +5 tests (757 → 762). Also in this followup: per-row click-to-copy bearer URL on `export_ready` /memlog rows, click-to-copy URL in the personal-archive `<details>` block (mirrors the rssvp-list pattern), `+ import sub` link surfaced from the home active-subs block (discoverability fix), `cursor: pointer` on the `.memlog-export` summary, and two new operator-side smoke scripts: `bin/m7-smoke-real.sh` (full HTTP round-trip across two real `bin/server.js` instances, complementing the existing worker-pipeline smoke) plus `bin/m7-manual-smoke-{up,down}.sh` for staging the dev pair on :8081/:8082.

### M8: Operator surface + UX polish [LOCKED]

Operator-facing: "is it running, is it backed up, will it come back, did anything change this week."
User-facing: light/dark toggle + sticky note per sub.
No clustering, no Docker images, no Prometheus exporter — operator brings their own init system; plato ships shell scripts and a healthz route.

- **B0 — light/dark theme toggle — SHIPPED.** Two-state button as the last item in the header right-cluster (anonymous + logged-in). Default = OS hint via `@media (prefers-color-scheme: light)`; once clicked, the choice is sticky in `localStorage` per browser and the OS hint stops applying. Anti-flash inline `<script>` in `<head>` stamps `data-theme` + `has-js` on `<html>` before first paint. Operator-overridable palettes: `branding.colors` (existing) for dark, new `branding.colorsLight` for light — same `resolveBrandingColors` validator, same `--up`/`--down` shape, both emitted as inline `<style>` blocks under the right scope. `bin/server.js` already forwards the entire `branding` block, so the operator just adds a `colorsLight` key in `config.json`. New `src/web/static/theme.js` (~30 LOC, defer-loaded). Without JS the button hides itself via `html:not(.has-js) .theme-toggle { display: none }` so no-JS users get the media-query behavior with no dead chrome. +3 tests covering the new field-name validation path on the second palette.

- **B1 — sticky note per sub — SHIPPED.** One mod-voice slot above the feed on `/sub/<name>`. Migration `023_sub_sticky_note.sql` adds `subs.sticky_note TEXT` (NULL = no note). New `validateStickyNote` + `setSubStickyNote` in `src/content/sub.js` mirror the description plumbing; empty string normalizes to NULL so the renderer's "is set" check is one branch. Render uses the existing `renderMarkdown` so the security posture (raw HTML escaped, image syntax → link, dangerous URL schemes filtered) is automatic. New `POST /sub/<name>/sticky` route, gated to any mod (owner or co — separate from `/edit`'s owner-only gate so co-mods retain the mod-voice without inheriting the settings surface), 401/404/403/409 (disabled sub) shaped like sibling routes, redirects to `/edit` on success. Sticky-note form added on `/edit` for both roles; owner-only block (description/flairs/threshold) unchanged. CSS adds `.sub-sticky-note` (warm-accent left rule, distinct from posts and from the sensitive/imported banners). +18 tests (5 unit on `validateStickyNote` + 13 integration: persistence, render-above-feed, image-stripping, all auth states, co-mod editability, empty-clears, length-cap, disabled-sub, edit-form prefill). 786 → 804 green. **Locks** (now in code, not just spec): one note per sub, ≤ 200 chars, no history, no algorithmic feed promotion, post-pinning stays permanently out.

- **B2 — `/healthz` endpoint — SHIPPED.** Public, unauthenticated `GET /healthz` returning JSON `{ok, version, uptime_s, db_writable, exports_dir_writable, last_migration}` with `Cache-Control: no-store`. Status `200` when both writability checks pass, `503` otherwise so an external watcher (B4, next) can alarm on it. `db_writable` runs `BEGIN IMMEDIATE; ROLLBACK;` so the probe acquires the same reserved lock real writes do — read-only filesystems and stale handles surface here. `exports_dir_writable` is `accessSync(W_OK)` against the configured `exportsDir`; not configured = `false`. `version` is read from `package.json` once at module load; `uptime_s` from `process.uptime()`; `last_migration` from `schema_migrations` tail (null when the table is absent — useful for fresh installs). +6 tests covering happy path, schema shape, the two failure modes, and last-migration plumbing. Operator-guide gets a `curl` one-liner under recurring operations.

- **B3 — backup script — SHIPPED.** _(Superseded in 0.12.6: `bin/backup.sh` snapshots both DBs via bundled `node:sqlite` `VACUUM INTO`. Retired in 0.12.14: replaced by pulselog's `--backup` mode — see CHANGELOG 0.12.14. The original milestone note follows.)_ `bin/backup.sh` (~90 LOC bash) snapshots an atomic `forum.db` via `sqlite3 .backup` — uses SQLite's online-backup API so concurrent writers don't corrupt the copy and the server stays up — alongside `posts/`, `exports/`, `config.json`, `spam-patterns.txt`, `data/urlhaus.txt`, `disposable-domains.txt`. Output: `$BACKUP_DIR/plato-backup-<YYYY-MM-DD-HHMMSS>.tar.gz`. Rotation keeps newest `$BACKUP_KEEP` (default 7), `set -e` clean, missing optional files (e.g. fresh install with no `config.json`) skipped silently. Off-host copy is a commented `rsync` stanza at the bottom — operator opts in by uncommenting and configuring their own SSH key; plato does not bundle key management. Operator-guide gets a recurring-operations entry with sample crontab + restore procedure. Verified end-to-end against the live :8080 instance: 876K archive, all expected files present, rotation trims to N as designed. No tests — bash-only, syntax-checked via `bash -n`; behavior verified manually.

- **B4 — health watcher cron — SHIPPED.** _(Retired in 0.12.14: `bin/health-watch.sh` replaced by pulselog's health mode — see CHANGELOG 0.12.14. The original milestone note follows.)_ `bin/health-watch.sh` (~140 LOC bash) curls `/healthz`, exits silent on `200` so cron stays quiet. On non-2xx (or curl failure → pseudo-status `000`): one-line stamp to `$BACKUP_DIR/health.log` (`<ISO> host=<h> status=<code> reason=<...>`), and if `$HEALTH_ALERT_EMAIL` is set, an email via `mail` or `sendmail` (whichever is on PATH; alert dropped with a stderr note when neither is installed). Email body sections: failure summary, first 20 lines of the response body (capped to keep HTML 404 pages from drowning out JSON), last 30 lines of `$PLATO_LOG`, last 5 rows of `health.log`, paste-ready GitHub issue template (title with version, project URL, repro placeholder, diagnostic block). The B2 surface is the readiness signal; B4 is the consumer that turns "non-2xx" into "operator wakes up with an issue body already assembled." Verified end-to-end against the live :8080 (which is still pre-B2 and returns 404 for `/healthz`) and against a closed port (curl-timeout path); both produced sane log lines + email bodies.

- **B5 — weekly stats report — SHIPPED.** _(Superseded in 0.12.14: `bin/stats-weekly.js` retired; pulselog's `--digest` mode now owns the history + WoW table + email, consuming `bin/stats.js --metrics-json` — see CHANGELOG 0.12.14. The original milestone note follows.)_ Two-script pipeline already in `bin/`: `bin/stats.js` (daily, ~58 LOC) appends one JSONL line to `data/stats.log` — `{snapshot_at, users, subs, posts, comments, votes}` — append-only, never rewrites history; `bin/stats-weekly.js` (~150 LOC) reads the log, groups by ISO week, picks latest snapshot per week, takes the most recent 4, renders a fixed-width WoW table, sends via `/usr/sbin/sendmail` to `config.json:operator.email` (the same address every other plato cron uses, no `STATS_EMAIL` env split needed). Plain-text email body — no HTML, no charts, no third-party deps. `--dry-run` flag on both for stdout-instead-of-mail. Sample crontab in `cron-jobs.md`: daily snapshot at 04:35, weekly digest Monday 06:00 UTC. This session: added `votes` (`SELECT COUNT(*) FROM votes` — vote toggle-off DELETEs the row, so total-row-count is the right "votes currently in effect" measure) to both the daily snapshot and the weekly table; legacy snapshots (pre-votes-column) render as `0` via `?? 0` fallback rather than `NaN`. Closed M8 by also unifying B4's alert-recipient resolution: `HEALTH_ALERT_EMAIL` env wins (lets you route healthz alerts to a dedicated PagerDuty inbox), falls back to `config.json:operator.email`, then no-email — the log stamp is the artifact either way.
...
**Post-M8 — evaluation Docker image — SHIPPED.** `Dockerfile` at repo root + `bin/docker-entrypoint.sh` first-boot script + `.dockerignore` + `.github/workflows/publish-eval-image.yml` (push to GHCR on `v*.*.*` tag, `latest` tag always tracks). Image is `node:22-alpine`-based, multi-stage (deps + final), 241 MB built, runs as the `node` user (uid 1000), and exposes 8080. The entrypoint generates a fresh `KNOWLESS_SECRET` on first boot (persisted to `/app/data/knowless-secret` if a volume is mounted there; ephemeral otherwise), runs migrations, then `exec`s `bin/server.js`. `KNOWLESS_DEV_LOG_LINKS=true` is preset so magic-link URLs print to `docker logs` — the eval workflow is "click log in, paste any email, copy the URL out of `docker logs`, click it." `PLATO_EVAL_BANNER=1` flips on a yellow strip at the top of every page that flags this as evaluation-only and links to operator-guide §Why no docker for production. Docker's `HEALTHCHECK` directive wires through `/healthz` (B2) so `docker ps` reports the readiness state plato itself reports. Verified end-to-end locally: image builds, container boots, all 23 migrations apply, `/healthz` returns `{ok:true,version:"0.1.0",...}`, banner renders on `/`, login POST emits a magic link to logs, Docker reports `healthy` after start-period. README gets a two-path "Try it" block — fast (one `docker run` line) and production-shape (`git clone` + `npm` + cross-link to the operator-guide section). +3 tests on the banner toggle (createApp option, off-by-default, on-everywhere when on, before-header positioning). 804 → 807 green.

The production deploy path stays no-docker. Operator-guide §Why no docker for production captures the reasoning: plato is single-process / single-SQLite / single-port; container orchestration adds friction for config, backups, healthz, and migrations without solving anything plato needs. The eval image is marketing/onboarding only.

**Post-M8 — co-tenant deploy mode for `bootstrap.sh` — SHIPPED (0.12.7).** Motivated by the terribic→gitdone consolidation (2026-05-24), where bootstrap had to be skipped entirely and nginx + cert + opendkim (additive-only) + cron + logrotate hand-rolled because gitdone already runs nginx + inbound postfix on the same host. `bootstrap.sh` now takes `--skip-nginx` (operator brings their own vhost + cert), `--skip-mail` (operator runs mail additively — bootstrap stops probing postfix/opendkim and points at the additive recipe instead of the destructive §5), and `--co-tenant` (both). The always-safe shared steps (`plato` user, `/opt/plato`, systemd unit, cron, logrotate, runtime dirs) run regardless; the closing summary adapts to the flags. (Note: the original backlog text claimed bootstrap "runs the deploy-guide mail step" — it never did; bootstrap only ever *warned* if the mail stack was missing. The genuinely destructive `inet_interfaces=loopback-only` step is the **manual** deploy-guide §5, which the new co-tenant appendix tells operators to replace with an additive opendkim key + `SigningTable` append.) Docs: deploy-guide "Appendix: Co-tenant deploy".
