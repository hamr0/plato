...
- **B5 — weekly stats report.** `bin/stats-weekly.js` (extending the existing daily `bin/stats.js` if it's lightweight enough; otherwise a sibling). Computes: handles, subs, posts, comments, votes (current totals + last-week deltas + week-over-week % change). Optional second-mode flag `--email` sends to `STATS_EMAIL` if set. Sample crontab: `0 9 * * 1 ...` (Monday 9am local). Plain-text email body, no HTML, no charts. Same "no third party" posture — `mail`/`sendmail` only.
