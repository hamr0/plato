...
- **Light-mode toggle — SHIPPED (M8/B0).** Two-state header button, last item in the right-cluster. Pre-click default follows `@media (prefers-color-scheme: light)`; post-click stamps `.theme-light` or `.theme-dark` on `<html>` and persists in `localStorage.theme` (`'light'` / `'dark'`), winning over the OS hint forever for that browser. Class-based, not attribute-based — iOS Safari has chronic CSSOM-invalidation bugs around `[attr]` selectors that surface as "post-navigation theme toggle fails to repaint." Anti-flash inline `<script>` reads localStorage before first paint so reloads don't strobe. No-JS users get the OS-hint behavior with the toggle button hidden. Active defaults: dark = **tokyo-night** (`#1a1b26` bg, `#c0caf5` text), light = **zinc-cool** (`#eef0f2` bg, `#202428` text). `style.css` ships nine drop-in dark presets (tokyo-night, github-dark, warm-amber, cool-cyan, mocha-purple, monokai-pro, nord, gruvbox-dark, night-owl) and five light presets (zinc-cool, github-light, notion-cream, solarized-light, stone-warm) — each is one commented `:root { ... }` line, copy-paste over the active block to swap. Operators also get `branding.colors` (dark) and `branding.colorsLight` (light) in `config.json` for vote-arrow color overrides per palette.
