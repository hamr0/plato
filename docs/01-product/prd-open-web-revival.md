...
- **Identity model — render-time bracket on every imported handle (M7 followup lock).** Imported handles are inserted as synthetic, permanently-unclaimable rows in the `handles` table (no email derivation path on this instance — nobody can ever log in as them). The DB pseudonym stays clean (e.g. `donkey-2x9k`); the bracket is **rendered at display time** by `pseudonymsByHandle` whenever `handles.imported_from_fingerprint` is non-null. Every render site — post author, comment author, modlog mod_handle, /memlog actor cell — surfaces `[donkey-2x9k]` so live readers get an unmissable "this user is from another instance, replies won't reach them" signal anywhere they appear, not only on the rare collision case. On pseudonym collision with an existing native handle, a numeric suffix is appended at storage time so the UNIQUE constraint holds (`donkey-2x9k` → `donkey-2x9k-2`), and the renderer brackets the disambiguated form (`[donkey-2x9k-2]`). Locked at render-time bracketing (vs storage-time bracketing) because the canonical pseudonym stays useful for URLs, search, and future @-mentions — the bracket is annotation, not name.
