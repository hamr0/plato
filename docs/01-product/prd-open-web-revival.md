...
Flairs are categorization slots, not content contracts. (The post's *contract* is its title, which freezes 15 minutes after posting; see *Permanently out → Editing post titles after the first 15 minutes*.) When a mod edits a sub's flair list:
...
- **Editing post titles after the first 15 minutes.** *(Amended 0.15.1 — was a blanket "titles are immutable". The reasoning below is unchanged; only its edge moved.)* Title is the post's *contract* — it shows in feeds, RSS, sub indexes, and archive snapshots, and it is what early voters cast on. A freely mutable title opens a 24h bait-and-switch lane (post a strong claim, harvest the votes, edit to something innocuous before the window closes) where the manipulation is invisible at the surface where the original title was read. Body edits are *local* by comparison: only readers who click through see them. So the rule is **a title is editable for 15 minutes after posting (`TITLE_EDIT_WINDOW_MS`), then frozen for good** — a much shorter leash than the 24h body/`sensitive` window, which keeps running independently. The carve-out exists because the original blanket lock leaned on an escape hatch plato doesn't have: it assumed "the cost of typo'd titles is small (delete and re-post)", but there is **no author-delete**, so a typo was permanent and un-fixable. 15 minutes is long enough to catch a typo and short enough that there is little to harvest before the title freezes.

  The gate is deliberately a **pure function of the clock**, not of engagement. An earlier cut of this locked the title on the first vote or reply; that was wrong twice over, and the reasons are worth keeping: `castVote` DELETEs the row when a voter toggles their vote off, so an engagement-derived lock silently *un*-locks itself when someone changes their mind ("locks for good" was simply false); and counting the author's own vote or self-reply froze titles that nobody else had ever seen, defeating the typo fix the carve-out exists for. A permanent fact cannot be derived from mutable state — it would need a stored `title_locked_at`, and the clock gives the same guarantee for free. The residual cost is accepted and named: **for 15 minutes, a bait-and-switch is possible.** The rejected alternatives still stand: full 24h title mutability (a genuinely exploitable harvest window), and "title edit emits a visible EDITED marker in feed listings" (plumbing without trust gain — a marker doesn't un-harvest the votes). `postTitleLocked(post, now)` is the gate; `editPost` throws once it trips and treats an absent `title` as "leave it alone", so the lock is enforced at the API boundary, not just by hiding the field in the UI. Comment titles don't exist (comments only have body), so this lock has no comment-side analogue.
...
- **A link to this forum's own pages, pasted as a full URL, opens in a new tab.** `renderMarkdown` decides `target="_blank"` from `outboundHost(href)`, which returns a host for *any* absolute `http(s)` URL and never compares it to the instance's own hostname. So a user who copies a post URL out of the address bar (the natural way to cite another thread) gets a link that is badged `↗ ownsub.com` and opens away from the page, even though it's an in-forum navigation. Relative links (`/sub/x`) behave correctly — this only bites the copy-paste path. The `↗` badge has mislabelled these as outbound since it shipped; 0.15.1 made the mislabel load-bearing by attaching navigation behavior to it. The fix is to thread the instance's `baseUrl` into the markdown renderer and treat same-host absolute URLs as internal, which the renderer currently has no access to — it is a config-plumbing change, not a one-liner, which is why it isn't in 0.15.1.
