...
### Post titles freeze 15 minutes after posting

A title is editable for **15 minutes** after the post lands (`TITLE_EDIT_WINDOW_MS`), then frozen for good. The 24h body/`sensitive` edit window (`EDIT_WINDOW_MS`) keeps running independently — the two clocks are unrelated, and a frozen title does not stop body edits.

The asymmetry is the point. A title is the post's **contract**: it shows in feeds, RSS, sub indexes, and archive snapshots, and it is what early voters cast on. Body edits are *local* by comparison — only readers who click through ever see them. A freely mutable title would open a 24h bait-and-switch lane (post a strong claim, harvest the votes, swap the title before the window closes) where the manipulation is invisible at the surface where the original title was read. That lane stays closed; see *Permanently out*.

Titles were immutable-from-submit until 0.16.0. That blanket lock leaned on an escape hatch plato doesn't have: it reasoned that "the cost of a typo'd title is small — delete and re-post," but there is **no author-delete**, so a typo was permanent and unfixable. 15 minutes is long enough to catch one and short enough that there is little to harvest before the title sets.

The gate is deliberately a **pure function of the clock, not of engagement**. An earlier cut locked the title on the first vote or reply and was wrong twice over, and the reasons are worth keeping so nobody rebuilds it: `castVote` DELETEs the row when a voter toggles their vote off, so an engagement-derived lock silently *un*-locks itself when someone changes their mind (making "locks for good" simply false); and counting the author's own vote or self-reply froze titles nobody else had ever seen, defeating the typo fix the window exists for. **A permanent fact cannot be derived from mutable state.** Expressing "someone actually acted on this" would need a stored `title_locked_at` column; the clock buys the same guarantee for free.

The residual cost is accepted and named: **for 15 minutes, a bait-and-switch is possible.** It is small (little accrues in 15 minutes) but it is not zero.

`postTitleLocked(post, now)` is the gate. `editPost` throws once it trips and treats an absent `title` as "leave it alone", so the rule holds at the API boundary, not merely by hiding the field — and the edit form renders a locked title as read-only text. Title edits run the same spam gates as a new post (link cap, pattern file, URLhaus); see *Spam Defenses*.
...
- **Notification modes** are pull-only by design:
  - **None (default)**: subscribe silently. Posts appear on the My Subs page when the user visits.
  - **Whole-instance RSS** (`/rss`, public): the firehose — latest 50 posts across every sub, no account. The logged-out reader's "what's new here" feed; also the home page's `<head>` autodiscovery target for anonymous visitors.
  ...
  - **All-subs + notifications RSS** (`/u/<token>/rss`, token-gated): the all-subs feed plus the user's memlog signals (replies to their comments, mod actions on their content) interleaved by time. This is the feed the home-page rssvp affordance points a **logged-in** reader at.
...
The outbound shapes plato supports are all pull:

- **Whole-instance RSS** at `/rss` — public, no account. The latest 50 posts across every sub, newest-first, merged into one Atom feed (same drama-shape exclusion as the per-sub feed: hard-removed **and** soft-collapsed omitted). The logged-out counterpart to the personal feed, so a reader can follow "what's new on this instance" without an account. A live sliding window recomputed per fetch — never a frozen list — so readers dedupe by entry id and surface only what's new to them.
...
The home page surfaces one **context-aware rssvp affordance** (at the end of the `// active subs · last 24h` header, plus a `<head>` alternate link): it targets the public `/rss` for a logged-out reader and the personal `/u/<token>/rss` for a logged-in one (falling back to `/rss` for a logged-in reader with no content yet). Same affordance, audience-appropriate feed.
...
Flairs are categorization slots, not content contracts. (The post's *contract* is its title, which freezes 15 minutes after posting; see *Content Model → Post titles freeze 15 minutes after posting*.) When a mod edits a sub's flair list:
...
- **Async + queued + off-peak.** Both kinds of export are produced in the background by `bin/run-export-queue.js`, default off-peak window 01:00–06:00 server time. One job per worker tick, sequential — no concurrency *within* a queue. (Export and import run as separate processes and can fire in the same tick; together with the live server they are the only `forum.db` writers, so writes rely on a 5 s SQLite `busy_timeout` to wait out a momentary write-lock collision rather than fail.) Operator-tunable via `EXPORT_OFFPEAK_START` / `EXPORT_OFFPEAK_END` (hour 0–23) or fully disable with `EXPORT_OFFPEAK_DISABLE=1`.
...
- **Source + verification (0.13, locked).** Server fetches `<url>` over HTTPS. **No file uploads.** Refuses non-200, non-tarball content-type, or oversize (config knob, default 500MB; the *decompressed* size is capped too, so a gzip bomb can't OOM the worker). Before inserting anything, the worker fetches the detached `<url>.sig` and the **origin's** `/.well-known/plato-pubkey`, confirms that key's fingerprint matches the manifest's claim, and verifies the Ed25519 signature over the gzipped bytes. Trust is anchored on the host the importer chose — not on `manifest.instance.base_url` or any other value self-asserted inside the (attacker-controllable) archive, which would be circular. A forged or tampered archive is refused before any DB write. Unsigned archives (`pubkey_fingerprint: null` — pre-signing exports or fork-stripped) are **refused by default**; `IMPORT_ALLOW_UNSIGNED=1` opts in for trusted legacy migrations. Refusal is reserved for *certain* failures: only a genuine `404`/`410` on the `.sig`/pubkey routes the archive to the unsigned path, while a transient fetch failure (network error, or a `5xx`/`429`/`403`) re-queues the job — a momentary outage on the source's well-known endpoint must not permanently fail a legitimately signed import, nor silently downgrade it to "unsigned." (Wired 0.13, security review — the signing infrastructure shipped in M7/B4 but the importer had never checked it.)
...
- **Async + queued + off-peak.** New `import_jobs` table mirrors `export_jobs`. New `bin/run-import-queue.js` worker runs in the same off-peak window as exports. Memlog notifications: `import_ready` on completion, `import_failed` with reason on terminal failure. **One import per account at a time (0.13):** the worker drains one job per tick, so a single in-flight slot per requester bounds the queue; a second request while one is pending or running returns 429 (closes the "vary the URL to bypass the `(source_url, requester)` dedupe and flood the queue" path — no new config knob).
...
**The gates cover the edit path, not just the submit path (0.16.0).** Rules 6, 6b, and 9 (link cap, URLhaus, pattern file) run on `POST /sub/<name>/post/<id>/edit` exactly as they run on `/draft` and `/finalize`: the cap rejects with a 400 before the write; the pattern and host matchers auto-collapse and system-flag after it; both match on `` `${storedTitle}\n${body}` ``. Until 0.16.0 the edit route ran none of them, so "publish clean, edit spam in" was a working bypass of the whole stack — a defense that only guards the front door is not a defense. Re-matching on every edit is idempotent by construction (collapse guarded on `collapsed_at IS NULL`; `flags` is `UNIQUE(target_type, target_id, flagger_handle)`), so a spam post that stays spam does not accumulate duplicate modlog rows.
...
- **Observability**: in-process error capture to a local JSONL (flightlog — uncaught exceptions, unhandled rejections, and request 500s, structured, `jq`/`tail` as the whole read surface; no aggregator, UI, or metrics endpoint) plus an out-of-process watcher (pulselog) on a schedule for health probes, a weekly stats digest, and nightly WAL-safe backups. Both are zero-dependency libraries and "the JSONL is the interface." The reference impl ships both **on by default**, autoconfigured from `config.json operator.{email,service}` with sound hobby-scale defaults; the operator surface is deliberately small — backup retention (`operator.backupKeepLast`) and the disk-alert threshold (`operator.diskMaxPercent`) are the only tunable knobs (validated at config-gen time), and `operator.monitoring: false` is the watcher's off-switch. flightlog (in-process) has no off-switch by design.
...
- **Light-mode toggle — SHIPPED (M8/B0; mobile-hardened in 0.10.1).** Two-state header button, last item in the right-cluster. **Light is the out-of-box default** (since 0.16.0 — the bare `:root` block carries the light palette; it was dark before). Pre-click, a visitor whose OS explicitly prefers dark gets dark via `@media (prefers-color-scheme: dark)` scoped to `:root:not(.theme-light)`; OS-light and OS-no-preference both resolve to light. Post-click stamps `.theme-light` or `.theme-dark` on `<html>` and persists to **both** `localStorage.theme` and a first-party `plato_theme=light|dark` cookie (`SameSite=Lax`, 1-year expiry), winning over the OS hint forever for that browser. Dual persistence is the fix for mobile Firefox session-clearing localStorage between refreshes — first-party cookies survive privacy modes that wipe localStorage. Class-based, not attribute-based — iOS Safari has chronic CSSOM-invalidation bugs around `[attr]` selectors that surface as "post-navigation theme toggle fails to repaint." Anti-flash inline `<script>` reads localStorage first, falls back to cookie, applies the class before first paint so reloads don't strobe. A `pageshow` listener gated on `event.persisted === true` re-syncs theme state on bfcache restoration (mobile Firefox pull-to-refresh path). No-JS users get the OS-hint behavior with the toggle button hidden. Known residual limitation: regular mobile Firefox F5/pull-to-refresh occasionally drops the persisted theme on some build variants — recoverable with one tap, every other interaction path is bulletproof. Server-side rendering of the theme class via cookie is the next step if needed; currently deferred since the workaround is trivial. Active palettes: light = **zinc-cool** (`#eef0f2` bg, `#202428` text) — the default, in `:root`; dark = **tokyo-night** (`#1a1b26` bg, `#c0caf5` text) — in `:root.theme-dark` plus the `prefers-color-scheme: dark` block (mirror both when swapping). `style.css` ships nine drop-in dark presets (tokyo-night, github-dark, warm-amber, cool-cyan, mocha-purple, monokai-pro, nord, gruvbox-dark, night-owl) and five light presets (zinc-cool, github-light, notion-cream, solarized-light, stone-warm) — each is one commented `:root { ... }` line, copy-paste over the active block to swap. Operators also get `branding.colors` (dark) and `branding.colorsLight` (light) in `config.json` for vote-arrow color overrides per palette.
  ...
  **Discoverability is declarative-only, and that's a posture, not an afterthought (see [`docs/04-process/privacy-seo.md`](../04-process/privacy-seo.md)).** plato is maximally findable through pure open-web machine-readability and nothing extractive: no analytics, no third-party JS, no tracking pixels, no tag managers. The surface, all dynamic and per-instance (derived from operator branding + base URL, so forks inherit it without editing): full head tags (`<title>`, description, canonical, theme-color, `html lang`, favicon, OpenGraph, `twitter:card`), `/robots.txt`, `/sitemap.xml` (every static page + sub + non-removed post, with `<lastmod>`), `/.well-known/security.txt`, `/humans.txt`, and per-sub Atom feeds with `<link rel="alternate">` autodiscovery. **As of 0.14.0**, three agent/LLM-readability additions: (1) **JSON-LD** — `WebSite` on the homepage, `DiscussionForumPosting` on each live post (removed posts emit none, mirroring the description fallback); this reverses an earlier "skip on principle" stance, justified because `application/ld+json` is parsed, not executed — same open-web tier as `<meta>`. (2) **`/llms.txt`** — a curated markdown index ("sitemap.xml for LLMs"): privacy invariant up top, links to chrome pages and the busiest public subs. (3) **Named AI crawlers in `robots.txt`** — retrieval/cite-live bots (`Claude-User`, `Claude-SearchBot`, `OAI-SearchBot`, `ChatGPT-User`, `PerplexityBot`) and training-corpus bots (`ClaudeBot`, `GPTBot`, `CCBot`) named explicitly so the decision is on the record rather than implied by `User-agent: *`. **Decision: all are `Allow: /`.** A plato instance is public forum copy meant to spread, and retrieval bots cite with a link back; the wildcard group's `Disallow` list still guards every private POST/auth/`/memlog`/token-gated `/u/` path. No config knob — the policy is baked in so every fork ships the same honest, on-the-record posture. The privacy claim that surfaces in the result snippet, OG card, and `llms.txt` ("magic-link auth, no tracking, no analytics; posts are markdown on disk") is a contract: it must stay in sync with what the code does.
...
- **A post title that stays editable, and an "EDITED" marker instead of a freeze.** Two rejected shapes of the same idea, both still rejected — but note what is *not* in this list: titles **are** editable for their first 15 minutes (0.16.0). That rule is shipped behavior and lives in *Content Model → Post titles freeze 15 minutes after posting*; it does not belong in a section about things that will never exist. What remains permanently out is (a) **title mutability for the full 24h body window** — a genuinely exploitable harvest lane (post a strong claim, collect the votes, swap the title before the window shuts, with the swap invisible at the surface where the original was read), and (b) **"a title edit emits a visible EDITED marker in feed listings"** — plumbing without trust gain, because a marker does not un-harvest the votes. Comment titles don't exist (comments only have body), so neither has a comment-side analogue.
...
A limitation graduates out of this list only when it is actually fixed, never when it becomes inconvenient. One already has: **the post-edit route bypassing every spam gate** (link cap, pattern file, URLhaus) was found by the 0.16.0 release gate and closed in 0.16.0 rather than documented and deferred — the 15-minute title window would have extended a pre-existing body-edit hole to the title, which is the field feeds and RSS show, and shipping a widened spam lane behind a paragraph of prose is exactly the failure this section exists to prevent. See §Spam Defenses.

- **A failed post edit discards the body you typed.** Every `editPost` refusal — blank body, over-cap body, closed 24h window, locked title, read-only sub — routes through `handlePostEdit`'s `errorPage`, which is a dead-end page. The submitted body is not carried back into a re-rendered form, so an author who writes 3 000 words and trips any one of those gates loses the text unless the browser's back button happens to restore it. The sharpest instance is the title lock: open the edit form at minute 14, submit a title fix plus a long body at minute 16, get a 403, lose both. This predates the 0.16.0 title window (blank-body has always behaved this way) — the title lock just added a gate that trips on a *clock*, so it can fire on a form that was legitimate when it was rendered. The fix is to re-render `renderPostEditPage` with the submitted values and the error inline, the same shape `postRetryView` already gives the submit path; it was left out of 0.16.0 to keep the title change small, not because the data loss is acceptable.

- **A link to this forum's own pages, pasted as a full URL, opens in a new tab.** `renderMarkdown` decides `target="_blank"` from `outboundHost(href)`, which returns a host for *any* absolute `http(s)` URL and never compares it to the instance's own hostname. So a user who copies a post URL out of the address bar (the natural way to cite another thread) gets a link that is badged `↗ ownsub.com` and opens away from the page, even though it's an in-forum navigation. Relative links (`/sub/x`) behave correctly — this only bites the copy-paste path. The `↗` badge has mislabelled these as outbound since it shipped; 0.16.0 made the mislabel load-bearing by attaching navigation behavior to it. The fix is to thread the instance's `baseUrl` into the markdown renderer and treat same-host absolute URLs as internal, which the renderer currently has no access to — it is a config-plumbing change, not a one-liner, which is why it isn't in 0.16.0.
