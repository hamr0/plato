...
- **Identity model — pseudonyms preserved, brackets on collision.** Imported handles are inserted as synthetic, permanently-unclaimable rows in the `handles` table (no email derivation path on this instance — nobody can ever log in as them). Pseudonyms are preserved verbatim *unless* they collide with an existing pseudonym on this instance, in which case the lexical part is wrapped in brackets: `donkey-2x9k` → `[donkey]-2x9k`. Suffix unchanged. The brackets visually mark "this name traveled" without inventing a new word.
