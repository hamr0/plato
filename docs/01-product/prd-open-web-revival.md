...
- **Identity model — pseudonyms preserved, full-bracket on collision (M7 followup lock).** Imported handles are inserted as synthetic, permanently-unclaimable rows in the `handles` table (no email derivation path on this instance — nobody can ever log in as them). Pseudonyms are preserved verbatim *unless* they collide with an existing pseudonym on this instance, in which case **the whole pseudonym is wrapped in brackets**: `donkey-2x9k` → `[donkey-2x9k]`. Further collision appends a numeric suffix: `[donkey-2x9k]-2`. Locked at full-bracket (vs the original lexical-only `[donkey]-2x9k`) because the bracketed token reads as one annotated unit rather than ambiguous half-decoration that could be misread as the user being literally named `donkey`.
