...
The bundled offline reader inside personal-archive tarballs (`kind=user` exports) auto-paginates when total items (posts + comments) exceed 100. Below the threshold it stays a single index page (current behavior preserved). Above it, the landing page becomes a chip navigator (`posts (N, Mp)` / `comments (N, Mp)` / one chip per `<year> (N)`) plus a "// recent activity" preview of the last 20 mixed items; subpages `posts.html`, `comments.html`, `<year>.html` render the filtered list paginated 100 per page (`posts-2.html`, `posts-3.html`, …) with prev/next links. No JS, no search — the reader's "fully offline, works in any browser" lock holds. Year pages combine posts + comments authored that year, newest-first.
