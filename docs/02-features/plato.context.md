...
The user-facing theme toggle (M8/B0) sits last in the header right-cluster. Two-state, no OS-default once clicked: a click stamps `.theme-light` or `.theme-dark` on `<html>` and persists in `localStorage.theme` (string `'light'` or `'dark'`). Anti-flash inline `<script>` in `<head>` reads the saved value and applies it before first paint so reloads don't strobe. Class-based, not attribute-based, because iOS Safari has chronic CSSOM-invalidation bugs around attribute selectors that surface as "first toggle works, post-navigation toggles fail" — class selectors don't have this problem in any engine. Without JS the button hides itself (`html:not(.has-js) .theme-toggle`) so no-JS users get the OS-hint behavior with no dead chrome.
