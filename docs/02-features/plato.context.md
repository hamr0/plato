...
| Daily full-state backup | install `bin/backup.sh` daily — tarballs `forum.db` + `knowless.db` + `posts/` (+ `config.json`, `spam-patterns.txt`) via bundled `node:sqlite` (no system `sqlite3` needed), newest `BACKUP_KEEP` kept (default 7). See [`cron-jobs.md`](cron-jobs.md) |
...
- Backups: run `bin/backup.sh` — it snapshots both `forum.db` and `knowless.db` via bundled `node:sqlite` (`VACUUM INTO`, WAL-safe hot copy, no system `sqlite3`) and tars `posts/` + config. Don't `cp` a live WAL DB; the `VACUUM INTO` snapshot is the safe path.
...
> A few recipes below use the `sqlite3` CLI for ad-hoc DB surgery. plato no longer installs it (the app and `bin/backup.sh` use bundled `node:sqlite`), so it's an optional operator convenience — `apt/dnf install sqlite3` if you want it. Note RHEL-family system builds (≤ 3.34) are too old to open plato's STRICT schema; use a recent `sqlite3` or a `node:sqlite` one-liner there.
...
bin/backup.sh                        # → backups/plato-backup-<timestamp>.tar.gz
