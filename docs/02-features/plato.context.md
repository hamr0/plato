...
| Tune backup retention / disk-alert threshold | `config.json` `operator.backupKeepLast` (default 7, 1–365) and `operator.diskMaxPercent` (default 90, 50–99) → `npm run gen-pulselog`. Set here, not in the generated `pulselog.config.json` (rewritten each run) |
...
| POST | `/sub/import` | enqueue a sub-import from a URL the user pastes (`sourceUrl` + optional `renameTo`). Auth-required; URL must be http(s); rename must match plato sub-name format. Idempotent on `(source_url, requested_by)` while pending; **429 if the account already has an import pending or in flight** (one at a time). The archive's signature is verified at worker time, not here. Redirects to `/memlog?import=queued`. M7/B5. |
...
- **Color tokens.** Every color in the UI is a `--*` CSS variable on `:root` in `style.css`. Re-skinning is a search-and-replace: `--bg`, `--text`, `--accent`, `--accent-warm`, `--border`, `--text-dim`. Vote arrows, links, logo dots, mod-button hover, modlog accent rows all re-skin together. **v1 requirement.** `style.css` also ships nine drop-in dark presets and five light presets — `tokyo-night` (active dark palette), `github-dark`, `warm-amber`, `cool-cyan`, `mocha-purple`, `monokai-pro`, `nord`, `gruvbox-dark`, `night-owl`; `zinc-cool` (active default — light ships out of the box since 0.15.1), `github-light`, `notion-cream`, `solarized-light`, `stone-warm`. Each is a single commented `:root { ... }` line at the top of `style.css`; copy any block over the active one and reload. The light palette is the default and sits in the bare `:root` block — one place. The dark palette sits under `:root.theme-dark` plus a `@media (prefers-color-scheme: dark)` mirror — both blocks must change together.
...
- Logging: plato writes to stdout/stderr. The deploy-shipped systemd unit redirects both to `/var/log/plato.log` (so journalctl shows lifecycle only; app output is in plato.log). knowless's mail-outcome hooks land here as `[plato mail.submit]` / `[plato mail.fail]` / `[plato mail.suppressed]` — grep-friendly. **Structured errors** (uncaught exceptions, unhandled rejections, request 500s — query string stripped so magic-link tokens never land) are appended to `data/logs/errors.jsonl` (gitignored) by [flightlog](https://github.com/hamr0/flightlog), an in-process recorder. `jq`/`tail` is the whole read surface; flightlog is deliberately not a logger/aggregator/UI. **Levers**: the sink path is the one operator-adjustable knob — `PLATO_FLIGHTLOG_FILE` (default `data/logs/errors.jsonl`). Disk is self-bounding and *not* a knob: plato doesn't pass `maxBytes`, so flightlog's 5 MB default rotation applies (rolls to `errors.jsonl.1`, current + one previous → trail caps at **~10 MB**); changing it is a `src/flightlog.js` edit, not config. The crash-policy flags (`exitOnUncaught`/`exitOnRejection`/`bootCheck`) and `0600` perms are hardcoded per-proc. There is **no off switch by design** — flightlog is zero-dep, in-process, append-only, logs only what we pass (no auto-harvest), and self-bounds, so the cost of leaving it on is ~nil; if you must silence it, relocate the sink, don't expect a toggle.
...
> A few recipes below use the `sqlite3` CLI for ad-hoc DB surgery. plato no longer installs it (the app and pulselog's backup engine use bundled `node:sqlite`), so it's an optional operator convenience — `apt/dnf install sqlite3` if you want it. Note RHEL-family system builds (≤ 3.34) are too old to open plato's STRICT schema; use a recent `sqlite3` or a `node:sqlite` one-liner there.
...
There's no UI for promote/demote yet (M5). For now, query the DB with plato's
own bundled engine — `node:sqlite` — so there's no system `sqlite3` CLI to
install (and the system CLI on RHEL-family distros is too old to open plato's
STRICT schema). Run from the install dir:

```bash
NODE_NO_WARNINGS=1 node -e 'const { DatabaseSync } = require("node:sqlite");
new DatabaseSync("forum.db").prepare(
  "INSERT INTO sub_mods (sub_name, handle, role) VALUES (?, ?, ?)"
).run("cooking", "<handle>", "co");'
```
...
```bash
NODE_NO_WARNINGS=1 node -e 'const { DatabaseSync } = require("node:sqlite");
const db = new DatabaseSync("forum.db", { readOnly: true });
console.table(db.prepare(`
  ...
  WHERE f.target_id IN (
    SELECT id FROM posts WHERE sub_name = ?
    UNION
    SELECT id FROM comments WHERE post_id IN (SELECT id FROM posts WHERE sub_name = ?)
  )
  AND f.resolved_at IS NULL
  ORDER BY f.created_at DESC
`).all("forever-friends", "forever-friends"));'
```
...
```bash
# backup — one command: both DBs (forum.db + knowless.db) + posts/ + config,
# snapshotted via bundled node:sqlite (no system sqlite3 needed)
node_modules/.bin/pulselog --backup --config pulselog.config.json   # → data/backups/plato-backup-<timestamp>.tar.gz

# restore on a new host
mkdir -p restore && tar xzf backups/plato-backup-*.tar.gz -C restore
cp restore/forum.db restore/knowless.db .
cp -r restore/posts .
