...
- **`RATE_LIMIT_FLOOR`** (`src/content/rateLimit.js`) — PRD-locked floor for per-account + per-sub rate limits. Operator can tighten via `config.json`; loosening throws at boot. **Owner carve-out**: when the poster owns the destination sub, the per-sub topic-flood cap is skipped (the cap defends against one account dominating one sub — meaningless when you own the sub). The global per-day cap (10/50/100 by tier) still applies, so an owner can't drain the day's quota across the instance. Wired in `handleDraft` and `handleFinalize` via `canModerate(...) === 'owner'`.
