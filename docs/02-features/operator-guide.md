...
**Daily backup.** `bin/backup.sh` snapshots `forum.db` + `knowless.db` + `posts/` + `config.json` + `spam-patterns.txt` to `$BACKUP_DIR/plato-backup-<timestamp>.tar.gz`. Both databases are snapshotted with plato's bundled `node:sqlite` (`VACUUM INTO`, WAL-safe) — no system `sqlite3` CLI dependency, so it works on any distro. Keeps the newest `BACKUP_KEEP` archives (default 7; drop to 4 if disk-tight); failures exit non-zero and surface via the cron `MAILTO`. See [`cron-jobs.md`](cron-jobs.md) for the crontab line.
...
- **Backups get awkward.** `bin/backup.sh` does an online `node:sqlite` `VACUUM INTO` against known file paths. Inside docker that's either `docker exec` from cron (couples cron to container lifecycle) or expose the file via volume (leaks the implementation detail you containerized to hide).
...
- **Backups (M8/B3; revised 0.12.6).** `bin/backup.sh` writes `BACKUP_DIR/plato-backup-<YYYY-MM-DD-HHMMSS>.tar.gz` containing online snapshots of **both** `forum.db` and `knowless.db` (via `bin/db-snapshot.mjs` — bundled `node:sqlite` `VACUUM INTO`, lock-safe, no need to stop the server, no system `sqlite3` dependency) plus `posts/`, `config.json`, and `spam-patterns.txt`. Regenerable caches (`exports/`, `data/urlhaus.txt`, `disposable-domains.txt`) are intentionally excluded. Tunables (env vars): `BACKUP_DIR` (default `./backups`), `BACKUP_KEEP` (default `7` — newest N kept, older deleted). Sample crontab entry — daily at 03:30, drops a tarball under `/var/lib/plato-backups/`:
