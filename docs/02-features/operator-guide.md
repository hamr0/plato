...
- **M6 — Subscriptions and notifications.** M6/B0: memlog (per-user notification log) shipped — `/memlog` route reachable from the pseudonym in the header, three kinds (`comment_on_post`, `reply_to_comment`, `mod_action`), unread chip, 90-day lazy-prune retention, no vote events. Still upcoming: subscriptions (subscribe/unsubscribe, `/subs?filter=mine` view, home-feed `subscribed | all` toggle), email digest, ntfy push, per-sub RSS feeds, outbound-mail signature.
