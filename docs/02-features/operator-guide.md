...
- Mobile-responsive layout at ≤640px: header stacks instead of overlapping, wide tables drop non-essential columns or scroll within their block, no full-page horizontal scroll on phones, code blocks and quotes soft-wrap to fit the column instead of overflowing.
...
| Preset | Bg | Vibe |
|---|---|---|
| **tokyo-night** *(active dark palette)* | `#1a1b26` | modern deep navy, soft blues |
...
| Preset | Bg | Vibe |
|---|---|---|
| **zinc-cool** *(active default — light ships out of the box)* | `#eef0f2` | soft cool gray, lowest brightness, terminal-honest |
...
The light palette is the default and lives in the bare `:root` block — swap it there, in one place. The dark palette lives under `:root.theme-dark` plus a `@media (prefers-color-scheme: dark)` mirror — both must change together if you swap dark. Look for the labelled comment line above each block.
...
| `operator.fallbackCommand` | optional | second, out-of-band alert path (pulselog ≥ 0.7.0) so a broken primary mail path can't also swallow the alert about it. When set, `gen-pulselog` adds a `fallback` sink to health/digest/backup; pulselog runs it with no shell, alert body on stdin, subject in `$PULSELOG_SUBJECT`. You supply the transport (plato bakes in none) — e.g. `"curl"` with `operator.fallbackArgs` posting to ntfy/Slack. Also: `operator.fallbackWhen` (`"always"` \| `"on-primary-failure"`, overrides the per-mode defaults) and `operator.fallbackTimeoutMs` (default 15000). Unset → no fallback (default) |
...
- `colors.up` (optional) overrides `--up` under `:root.theme-dark` and the `@media (prefers-color-scheme: dark)` block (dark theme) — positive score color and the voted-up arrow's "you voted here" memory shade.
- `colors.down` (optional) overrides `--down` under the same two dark scopes — negative score color and the voted-down arrow's "you voted here" memory shade.
- `colorsLight.up` / `colorsLight.down` (optional, M8/B0) override the same two variables under `:root` — light is the default palette, so it needs no class or media scope. (`colors` — the dark pair — lands under `:root.theme-dark` plus the `@media (prefers-color-scheme: dark)` block.) Independent of `colors`; either or both may be set.
...
**Nightly backup.** pulselog's `--backup` mode snapshots `forum.db` + `knowless.db` + `posts/` + `config.json` + `spam-patterns.txt` to `data/backups/plato-backup-<stamp>.tar.gz`. Both databases are dumped with plato's bundled `node:sqlite` (`VACUUM INTO`, WAL-safe) — no system `sqlite3` CLI dependency. Keeps the newest 7 (`backup.keepLast` in the generated config); a failed run exits 1 loud. On by default; see *Monitoring & backups* below and [`cron-jobs.md`](cron-jobs.md).

**Weekly stats digest.** pulselog's `--digest` mode (Mon 06:00 UTC → operator email). It runs `bin/stats.js --metrics-json` for the counters — users (`knowless.db.handles`: anyone who's ever requested a magic link), subs, posts, comments (excluding `removed_at`), votes — appends one snapshot to `data/logs/stats.jsonl`, and emails a fixed-width 4-week WoW table plus a 7-day flightlog error rollup. `pulselog --digest --dry-run` prints to stdout for local testing. See [`cron-jobs.md`](cron-jobs.md).
...
**Site rules (`branding.rules`).** Optional array of up to 4 short strings (joined ≤240 chars, printable ASCII only, no URLs and no bare domains). Rendered as a list on `/about` AND injected into the magic-link email signature so users see the same text in the medium they actually read. Single source of truth — edit one config field, both surfaces stay in sync. The URL ban covers any `[a-z]+://` scheme (`http`, `https`, `mailto`, `data`, `ftp`, …) AND bare domain shapes (`example.com`, `host.io/path`) that mail clients auto-link — a phishing-vector defence on the email signature.

**Magic-link mail body shape (0.10.3).** The body now reads in this order: security warning ("expires in 15 minutes / ignore this email") → `Click to sign in:` → URL → optional `Last sign-in: <ISO ts>` block (when the handle has a prior login) → `--` signature delimiter → optional instance stub (`a plato instance hosted by @<hostedBy> . <feedbackEmail>`, both halves degrade gracefully) → civility rules (≤4 lines from `branding.rules`). Composed via knowless's `bodyOverride` mechanism — plato owns the entire body, including the footer block, so the legacy `validateBodyFooter` 4-line/240-char cap doesn't apply (only the broader 2048-char `bodyOverride` cap, which the worst-case body comes nowhere near). Operators who want to inspect the live shape: send yourself a magic link and read the email; tests in `test/integration/mail-body.test.js` pin the order.
...
The `established` per-account ceiling (20 posts/day, 60 comments/day) caps cross-sub fan-out: a single voice can't dominate the home feed by spreading posts across many subs. Per-sub flood cap and per-account cap apply independently — the binding constraint is whichever bites first.

**Owner carve-outs (in own sub only)**:

| Tier | posts/day in own sub (owner) | comments/day in own sub (owner) |
|---|---|---|
| new         | 3 (unchanged — brigade guard) | 20 (2×) |
| recent      | 20 (2×) | 60 (2×) |
| established | 40 (2×) | 120 (2×) |

The new-tier post asymmetry is deliberate: doubling 3 → 6 here would open the "fresh account → fresh sub → flood seed posts" brigading vector. New-tier *comments* are still doubled because comment-flooding-own-sub is engagement, not brigading. Per-hour cap and per-sub topic-flood cap are also skipped for owners in their own sub. Doubled budget is "spent in own-sub only" — once dayCount exceeds the base cap, posting elsewhere fails on the lower base cap check.
...
| Window | Duration |
|---|---|
| Post / comment edit window | 24h after creation (body + `sensitive`) |
| Post title edit window | 15 min after posting (`TITLE_EDIT_WINDOW_MS`), then frozen — titles are the post's contract |
...
- **No default sub on fresh installs.** Migration 002 created a `general` sub as the FK target for backfilling M1-era posts; migration 024 drops it on installs where no posts ever landed there. Old instances with real M1 archives keep `general` for archaeology (read-only; new posts must land in a real sub with a real owner-mod). Reddit's `r/all` problem shouldn't be re-imported.
...
- **Backups get awkward.** pulselog's backup does an online `node:sqlite` `VACUUM INTO` against known file paths. Inside docker that's either `docker exec` from cron (couples cron to container lifecycle) or expose the file via volume (leaks the implementation detail you containerized to hide).
...
The recommended production path is: **systemd unit + cron + logrotate + the `bin/*.sh` scripts in this guide.** Any VPS with Node ≥ 22.5 has everything plato needs — `nvm install 22` is the entire prep. SQLite ships bundled in Node (`node:sqlite`), used for both the app and backups, so there's no system `sqlite3` package to install.
...
- **Cron jobs** — install the root crontab block from [`cron-jobs.md`](cron-jobs.md) once per instance. Two kinds: **[pulselog](https://github.com/hamr0/pulselog)** (the external watcher — health every 5 min, weekly stats digest, nightly backup) and **plato's own scripts** (hourly URLhaus refresh, quarterly disposable-domains refresh, **daily sub-inactivity sweep**). pulselog's `pulselog.config.json` is generated from `config.json` via `npm run gen-pulselog` — all three modes autoconfig from `operator.{email,service}`. **On by default; `operator.monitoring: false` turns pulselog off.** See *Monitoring & backups* below.
- **Archive export queue (M7/B2-a + B2-b)** — `bin/run-export-queue.js` runs as a separate cron, suggested `*/15 * * * *`. Picks one pending export job per tick (sub or user kind) and only does work between `EXPORT_OFFPEAK_START` (default `01`) and `EXPORT_OFFPEAK_END` (default `06`) server time. To widen the window on an idle VPS, override the env vars (hour 0–23) or set `EXPORT_OFFPEAK_DISABLE=1` to ignore the gate. Output lands in `./exports/` (gitignored). Per-kind windows (locked floors): production SLA is **7 days for sub, 3 days for user** from request — past that the worker terminal-fails with `exceeded SLA window` so the queue can't grow unboundedly; download TTL is **3 days for both**, picked for disk-pressure not policy. Both sweeps run at the start of each tick. Archives self-describe their origin from `branding.forumName` + `branding.baseUrl` — set `branding.baseUrl` in `config.json` so the README and index.html include a working link back to the live instance. Suggested crontab line:
  ...
  */15 * * * * cd /opt/plato && node bin/run-export-queue.js >> /var/log/plato-export.log 2>&1
  ```
- **Archive import queue (M7/B5)** — `bin/run-import-queue.js` runs on the same off-peak schedule as exports. Picks one pending import per tick, fetches the URL the user pasted (size cap `IMPORT_MAX_BYTES` default 500MB, applied to both the compressed download and the *decompressed* tar so a gzip bomb can't OOM the worker; fetch timeout `IMPORT_FETCH_TIMEOUT_MS` default 120s), parses + verifies per-file SHA-256s, **verifies the archive's Ed25519 signature** against the key the source URL's origin publishes at `/.well-known/plato-pubkey` (refuses a forged or tampered archive before any insert; unsigned archives are refused by default — set `IMPORT_ALLOW_UNSIGNED=1` to accept pre-signing/fork-stripped legacy archives), checks idempotence (same source archive can only succeed once via the manifest's scope.sub + exported_at), runs the insert in a transaction, emits memlog `import_ready` or `import_failed`. Off-peak window overridable via `IMPORT_OFFPEAK_START` / `IMPORT_OFFPEAK_END` (hour 0–23) or fully disable with `IMPORT_OFFPEAK_DISABLE=1`. Per-job retry up to 3 attempts then terminal-fail; 3-day SLA sweep on stuck rows. One import per account at a time (a second request while one is in flight returns 429). Suggested crontab line:
...
- **Monitoring & backups — pulselog (0.12.14).** plato ships [pulselog](https://github.com/hamr0/pulselog) (the external sibling to flightlog) as the watcher, **on by default** with safe defaults that mirror the bespoke scripts it replaced (`health-watch.sh` / `stats.js`+`stats-weekly.js` / `backup.sh`). `pulselog.config.json` is **generated from `config.json`** — `npm run gen-pulselog` — so the email / base URL / paths live in one place. Three cron modes (full crontab in [`cron-jobs.md`](cron-jobs.md)):
  - **health** (every 5 min) — probes localhost `/healthz`, disk, backup freshness, `plato.service`. Silent on green; one email on break, with the last 20 flightlog error lines pasted in (`alert.logTail` → `errors.jsonl`). Exits 0 on a health failure (the email is the signal). TLS-cert expiry stays on the daily `bin/check-cert.sh` (graduated severity; a 5-min cadence would re-email for the ~2 weeks before expiry).
  - **digest** (weekly) — runs `bin/stats.js --metrics-json` for `{users,subs,posts,comments,votes}`, appends to `data/logs/stats.jsonl`, emails a WoW table + a 7-day flightlog rollup grouped by `proc`.
  - **backup** (nightly) — `VACUUM INTO` of **both** `forum.db` + `knowless.db` (in-process `node:sqlite`, WAL-safe) + `posts/` + `config.json` + `spam-patterns.txt` → `data/backups/plato-backup-<stamp>.tar.gz` (`0600`), newest 7 kept. Exits 1 loud on failure.

  **What's default vs adjustable vs off:** *Default-on, no input needed* — health probes, backup scope/retention. *Adjustable via `config.json`* — `operator.email` (alert/digest/backup recipient; unset → log-only, no mail), `operator.service` (unit name + restart target). *Adjustable in the generated `pulselog.config.json`* — cadences, thresholds (`disk.maxPercent`), retention (`backup.keepLast`/`keepDays`), extra `command` checks; re-runnable by hand or regenerated. *Off* — set `operator.monitoring: false`; `gen-pulselog` then writes nothing and you skip pulselog's crontab lines. Off-host copy of the archive stays yours (a `pull`, not a push — see pulselog's `examples/pull-restricted.sh`); plato doesn't bake in key management.
- **Restoring from a backup.** stop the server → `tar -xzf plato-backup-*.tar.gz -C /opt/plato/restore` → copy `restore/forum.db` + `restore/knowless.db` into place over the live ones → copy `restore/posts/` into place → (optional) restore the operator-edited config + spam patterns → make sure `KNOWLESS_SECRET` matches the original deploy (otherwise identity hashes shift and every user looks like a new account) → `npm start`.

### When something goes wrong

**Filing an issue.** plato lives at <https://github.com/hamr0/plato/issues>. pulselog's health-alert email summarizes the failing checks and pastes recent flightlog errors — copy the relevant lines into a new issue. For other failures (a stack trace from `npm start`, a script erroring under cron) the same template works: include plato version, host, what you did, the diagnostic block. Smaller, specific issues get fixed faster than "everything is broken."
...
|---|---|---|---|---|
| `data/logs/health.jsonl` | pulselog health | one `kind:health` line per failing check | one per failure event — silent on green | self-rotating (`maxBytes`); `0600` |
| `data/logs/stats.jsonl` | pulselog digest | one `kind:stats` snapshot line per run | ~52 lines/year | not pruned (the trend record); tiny |
| `data/logs/backup.jsonl` | pulselog backup | one `kind:backup` line per run (`status`, `bytes`, `kept`) | one per nightly run | not pruned; tiny |
| `data/backups/plato-backup-*.tar.gz` | pulselog backup | tarball per run | one per nightly run | rotated to newest 7 (`backup.keepLast`) |
| `data/logs/errors.jsonl` | [`flightlog`](https://github.com/hamr0/flightlog) via `src/flightlog.js` | one JSON line per uncaught exception, unhandled rejection, or failed request/job — from the server **and** every cron/queue worker (`kind` + stack + `{app,release,proc,where}`); `proc` names the source (`server`, `export-queue`, `import-queue`, `ots-upgrade`, `inactivity`, `urlhaus`) | self-bounded ~2× 5 MB (rotates to `.1`) | self-rotating, zero config; `0600` perms |
...
- **M7 — Archives + portability [SHIPPED].** Canonical archive format with stable JSON shapes (M7/B1), async export-job queue + per-sub archive builder (M7/B2-a), HTTP routes + UX + memlog wiring + personal export (M7/B2-b), Ed25519 archive signing with `.sig` route + `/.well-known/plato-pubkey` for verification (M7/B4), URL-fetch sub-import with `[imported]` modlog tags + bracket-collision pseudonyms (M7/B5), operator-opt-in OpenTimestamps anchor via `ots` CLI shell-out — zero new deps (M7/B6). Followup polish: imported-author render lock (dim+italic + aria-label + persistent `[i]` chip), sub-archive paginated static reader for large archives (>100 posts gets year/post chips + paginated subpages), `/memlog` filter overhaul (cross-side chips with `notifKinds` + `contentKinds`, auto-flip empty unread → read history, modlog-style `showing: X · clear all` footer), sub-archive export and import surfaces in public modlog. Personal exports stay private; sub exports + imports are visible in the modlog; failed jobs do not appear.
- **M8 — Operator surface + UX polish [SHIPPED].** Light/dark theme toggle with operator-overridable light palette, anti-flash inline `<script>` to stamp saved theme before first paint, no-JS hides toggle (M8/B0). Sticky note per sub — single mod-voice slot above the feed, mods of the sub edit it from `/sub/<name>/manage` (M8/B1). `/healthz` operator readiness probe returns JSON `{ok, version, uptime_s, db_writable, exports_dir_writable, last_migration}` with `Cache-Control: no-store`; 200 when both writability checks pass, 503 otherwise (M8/B2). `bin/backup.sh` local snapshot script — online `node:sqlite` `VACUUM INTO` of both `forum.db` + `knowless.db` + `tar` of posts dir into rotating archives, newest 7 kept (M8/B3; revised 0.12.6 to cover both DBs and drop the system `sqlite3` dependency). `bin/health-watch.sh` cron-side `/healthz` watcher — silent on 200, on non-2xx writes a one-line stamp + emails operator with paste-ready GitHub issue body (M8/B4). Daily stats snapshot + weekly digest with WoW Δ formatting (M8/B5). Locks: `TITLE_MAX = 300`, `BODY_MAX = 40000`, `COMMENT_BODY_MAX = 10000` PRD-locked at Reddit's exact numbers. Comment "read more / show less" toggle native via `<details>` + CSS. Import-dedupe collisions now fail terminal on first attempt instead of swallowing for three retries.
- **Post-M8 — Canonical deploy + version visibility + mobile pass [SHIPPED].** Comprehensive [Deploy Guide](deploy-guide.md) for Ubuntu 24.04 LTS + nginx + Let's Encrypt + postfix + opendkim + systemd + cron, validated end-to-end against the first canonical plato deploy at terribic.com (RackNerd VPS + Route 53 + Gmail recipient): SPF + DKIM + DMARC all-pass at Gmail; Step 14 inbound aliases (`abuse@` / `postmaster@` / `feedback@` / `security@` via `virtual_alias_maps` + MX); Step 15 reputation-monitoring signups (Google Postmaster Tools, MS SNDS, MS JMRP). knowless+postfix+opendkim mail unification replaces the previous msmtp wrapper. Evaluation Docker image at `ghcr.io/hamr0/plato:latest` with yellow eval-banner strip and `PLATO_EVAL_BANNER=1` toggle — one-command localhost trial. Version visibility on three surfaces: footer (`· v<version>` after `/modlog` link), startup log line (`plato v<version> on <url>` to `/var/log/plato.log`), README shields.io badge auto-reading from package.json. Mobile-responsive pass: home-nav filter chips snake-wrap (`.filter-group { display: contents }`), login-form viewport-anchored on narrow screens (`position: fixed` left+right pinned), header column-stack on mobile (brand block on top, status row tucked right), post-page grid overflow fix (`grid-template-columns: auto minmax(0, 1fr)` + `min-width: 0` on `.body` + `overflow-wrap: anywhere` on `.post-title-line h1`), brand-area post-title truncated to first 3 words (full title still in `<title>` and article body). Apache 2.0 `LICENSE` in repo + SPDX `"license": "Apache-2.0"` field in `package.json`. Rate-limit floor extension: `established` tier capped at 6 posts/hr / 20 posts/day / 60 comments/day (was uncapped — closes the cross-sub fan-out hole); per-day post cap doubled for `recent` + `established` owners in their own sub (10→20, 20→40); new-tier owner stays at 3/day as a brigade-guard against "fresh account → fresh sub → flood seed posts."
...
A: Layered defense: (1) `disposable-domains.txt` blocklist at form submit, (2) magic-link requires a working inbox per identity, (3) account-age tiered rate limits (1 post/hour / 3 posts/day for new accounts <24h, 3/hr & 10/day for 1–7d, 6/hr & 20/day for established >7d) — **owners are exempt from the per-hour burst cap when posting in their own sub** and the daily post cap is **doubled** for `recent` and `established` owners in their own sub (10→20, 20→40); the new-tier daily post cap stays at 3 even for owners as a brigade-guard against "fresh account → fresh sub → flood seed posts" — and **the daily comment cap is doubled across every tier when commenting in a sub you own** (20/60/120) so owners can lead discussion without becoming tyrants, (4) per-sub topic-flood limit (5 posts/day per sub for accounts <30d, 20/day for trusted ≥30d) — **owners are exempt from this cap when posting in their own sub** since topic-flooding a sub you define is a contradiction, (5) per-post outbound link cap (1 link/post for new accounts), (6) `spam-patterns.txt` regex match → auto-collapse for mod review, (7) URLhaus hourly cron blocks known-malicious link hosts. New-account vote-weight rules (half weight, no comment voting, posts < 24h only) limit damage from any sockpuppet that gets through.
