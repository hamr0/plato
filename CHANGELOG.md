...
### Added — `dir="auto"` on user-content surfaces so Arabic / Hebrew / Persian posts render RTL while Latin / CJK stay LTR

Six render sites (post-page `<h1>` title, post-page `<article>` body, feed-view `<h2>` titles, feed-view body excerpts, comment-page `.comment-body`, comments-tab preview `.comment-body`) now carry `dir="auto"`. The browser detects writing direction from the first strong character per element, so Arabic / Hebrew / Persian / Urdu auto-flip to RTL while Latin / CJK / Cyrillic / Greek / Devanagari etc. stay LTR. Paired with `article > *, .comment-body > *, .preview > * { unicode-bidi: plaintext; }` in `style.css` so each block-level child of a markdown-rendered body detects independently — a post mixing English and Arabic paragraphs renders each paragraph in its own direction. UI chrome (header, action pills, vote arrows, modlog labels) stays LTR by design; this is a *post-anything-language* change, not a localization project. PRD §Permanently out now documents the boundary: forum content is multilingual, forum voice is English. Style version bumped to v=42 to bust the CSS cache. Test in `seo.test.js` pins `<h1 dir="auto">` and `<article dir="auto">` on the post page so future edits don't silently regress.
