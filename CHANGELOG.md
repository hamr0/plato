...
- **The post-edit route now runs the same three spam gates as the submit route.** `handlePostEdit` received `{ db, auth, postsDir }` and nothing else, so it ran **no** link cap, **no** spam-pattern match, and **no** URLhaus check — "publish something clean, then edit the spam in" walked past all three. The hole predates this release for the post *body*; the 15-minute title window would have widened it to the **title**, which is the field feeds, RSS, and sub indexes actually show. Both halves are closed together: the route now threads `spamPatterns` / `linkCapConfig` / `urlhausHosts` and checks `` `${title}\n${body}` `` in the submit path's exact order — link cap rejects with a 400 *before* the write, then patterns and URLhaus auto-collapse + system-flag *after* it. Re-running the matchers on an edit is safe by construction: the collapse is `WHERE collapsed_at IS NULL`-guarded and `flags` carries `UNIQUE(target_type, target_id, flagger_handle)`, so an already-collapsed spam post can't accumulate duplicate modlog rows. The title used for matching is the one that was **stored**, not the one submitted — a locked title leaves the submitted value on the floor, and scanning text the post doesn't carry would be theatre. Found by the release gate's `/security` pass on this diff, not in the wild. Validated end-to-end against a live server, not only in unit tests: a clean post, published and then edited, comes back collapsed with a `system`-attributed `pattern: guaranteed returns` modlog row when the spam lands in the **title**; collapsed when a URLhaus host lands in the **body**; and rejected with a `400 too many links` and `edited_at` still `NULL` when the body goes over the tier cap — the last one proving the cap refuses *before* the write rather than cleaning up after it.
- **Control-character-obfuscated `javascript:` links in markdown are now filtered (stored XSS).** `safeHref` (`src/content/markdown.js`) classified a URL scheme with two regexes, but a raw TAB (or other C0 control) inside the scheme — reachable via CommonMark's angle-bracket destination syntax, `[x](<java⟨TAB⟩script:…>)` — slipped past both: the dangerous-scheme regex `^[a-z][a-z0-9+.-]*:` doesn't match `java⟨TAB⟩script:`, so the href passed through with the control byte intact. A browser's URL parser strips tab/CR/LF while resolving the scheme, so the anchor navigates as `javascript:…` and runs attacker JS on the forum origin the moment a reader clicks — persistent, because it lives in stored post/comment/sticky-note content. Fixed by stripping C0 controls + DEL (`[\x00-\x1f\x7f]`) before the scheme test, exactly the bytes the browser strips, so `java⟨TAB⟩script:` normalizes to `javascript:` and is then correctly rejected. Legitimate URLs never carry raw control bytes, so nothing valid is lost. Regression test covers TAB/LF/CR/NUL/VT/FF. Found by the `/security` repo sweep.
- **Request bodies are capped at 1 MiB (`readBody`, `src/web/request.js`) — unauthenticated memory-exhaustion DoS.** `readBody` buffered the entire request stream with no ceiling, and every POST handler — including the unauthenticated `POST /login` and anonymous `POST /draft` — reads the body before any auth or validation. A few concurrent multi-GB POSTs would OOM the single process and take the whole forum down. Now capped at 1 MiB (≈25× the 40 000-char body max); on overflow the socket is destroyed and the read throws, which the dispatch try/catch turns into a 500 for the abuser. Found by the `/security` repo sweep.
- **Anonymous `POST /draft` is now capped per IP.** `submitDraft` wrote its row *before* knowless's per-IP login gate, so past the email cap an IP could still flood orphaned draft rows (no email, pruned at 24h — a slow disk-flood). The row-write now shares knowless's exact budget: a per-IP hourly counter seeded from `auth.config.maxLoginRequestsPerIpPerHour` (default 30) — **no new tunable**. Kept in-memory (per-app-instance), not a table or a `drafts.ip` column, so plato continues to store **no IP at rest** (identity stays HMAC-only); the single-process design makes the map coherent and a restart clearing it is normal rate-limiter behavior. Over the cap returns a deliberately vague `429` (0.10.4 UX-honesty rule — no cap/window leak). Found by the `/security` repo sweep.
- **Sub creation is capped at 5 *active* subs per owner.** One account could create unbounded subs (namespace squatting + seed-flood surface, plus unbounded growth of the `LIMIT`-less `listPostableSubs` / sitemap scans). `createSub` now refuses a 6th *active* sub (`MAX_ACTIVE_SUBS_PER_OWNER`, `src/content/sub.js`). Only active subs count — a disabled sub (30-day inactivity, or hand-off) frees a slot, so ditching a sub you tried and moved on from is never penalized. Enforced inside the mechanism, so every creation path is bound. Deliberately a plain structural cap, not an operator-tunable floor-model knob. Found by the `/security` repo sweep.
- **`/sitemap.xml` post enumeration is bounded at the 50 000-URL sitemap ceiling.** The post loop had no `LIMIT`, so an unauthenticated hit scanned and materialized the entire `posts` table into one string — a memory/CPU amplifier that grows with the archive. It now caps at `50000 − urls.length`, newest-first (Google already has the older permalinks; the sitemap is a crawl hint, not the archive), cached `max-age=300`. The cap is clamped with `Math.max(0, …)` because SQLite reads a **negative** `LIMIT` as *unlimited*: subs are disabled, never deleted, so on a long-lived instance the static + per-sub URLs alone could reach 50k and flip an unclamped subtraction negative — silently re-opening the very scan the cap exists to close. Found by the `/security` sweep; the clamp by the release `/code-review`.

### Added

- **A public whole-instance RSS feed at `/rss`, and a context-aware "rssvp" affordance on the home page.** Until now the only aggregate feed was the token-gated personal one (`/u/<token>/rss`) — a logged-out reader who wanted "everything happening here" had to add each `/sub/<name>/rss` by hand. `/rss` is the public firehose: the latest 50 posts merged across every sub, no account, same Atom renderer and same drama-shape exclusion (hard-removed **and** soft-collapsed omitted) as the per-sub feed. It's a live sliding window recomputed per fetch (never a frozen list); readers dedupe by entry `<id>` = permalink, so they still surface only what's new to each reader. The home page's `// active subs · last 24h` header now carries a right-aligned **rssvp** link at the other end of the line, plus a `<head>` `<link rel="alternate">` for reader-extension autodiscovery. Both are **context-aware**: a logged-in reader gets their personal feed (posts across their subs + notifications on their own content); a logged-out reader — or a logged-in one who has never posted or subscribed (no `handles` row yet) — gets the public `/rss`. Per-sub feeds and the personal `/memlog` feed are unchanged. Consistent with PRD §Outbound channels (pull-only); completes the channel rather than adding a new class of surface.

### Changed

- **Light is now the out-of-box theme.** The bare `:root` block in `style.css` carries the light palette (zinc-cool); dark moved to `:root.theme-dark` **plus** a `@media (prefers-color-scheme: dark)` block scoped `:root:not(.theme-light)`. Net behavior: OS-dark visitors still land on dark, OS-light *and OS-no-preference* now land on light (previously no-preference fell through to dark), and the header toggle still wins over both, persisted in `localStorage` + the `plato_theme` cookie. `theme.js`'s `effective()` fallback flipped to match (only an explicit `prefers-color-scheme: dark` yields dark). `themePaletteOverrides()` in `app.js` inverted its scoping in step, so operator `branding.colors` (dark) / `branding.colorsLight` (light) vote-arrow overrides still land in the right palette. Operators swapping palettes: light is now a one-block edit in `:root`; a dark swap must be mirrored into both dark blocks.
- **Outbound links in posts and comments open in a new tab.** The markdown renderer adds `target="_blank" rel="noopener"` to absolute `http(s)` links — the same set that already earns the `↗ host.com` outbound-host badge, so the "this leaves plato" signal and the new-tab behavior come from one test. Relative links (`/sub/x`), fragments (`#comments`), and `mailto:` stay same-tab. `rel="noopener"` blocks the opened page's `window.opener` access; referrer and SEO behavior are unchanged (no `noreferrer`/`nofollow`).
- **Post titles are editable for 15 minutes after posting, then frozen** — *amends the PRD's blanket "Editing post titles" lock*, which previously made titles immutable from submit. The rule now lives in **PRD §Content Model → "Post titles freeze 15 minutes after posting"**, because a thing that is *allowed for 15 minutes* cannot honestly sit in a section called §Permanently out; what stays permanently out is the part that actually is — full 24h title mutability, and a feed-visible EDITED marker. The lock's rationale is unchanged and still enforced: a title is the post's contract with everyone who voted or replied on the strength of it, so it must stop moving early. What moved is the edge — the original lock leaned on an escape hatch plato doesn't have. It assumed "the cost of a typo'd title is small (delete and re-post)", but there is **no author-delete**, so a typo was permanent and un-fixable. 15 minutes (`TITLE_EDIT_WINDOW_MS`) is long enough to catch one and short enough that there is little to harvest before the title freezes; the 24h body/`sensitive` window keeps running independently. `postTitleLocked(post, now)` is the gate — a **pure function of the clock, deliberately not of engagement**. An engagement-derived lock ("locks on the first vote or reply") was implemented first and was wrong twice over: `castVote` DELETEs the vote row on toggle-off, so the lock silently un-locked itself whenever a voter changed their mind, and counting the author's own vote or self-reply froze titles nobody else had ever seen. A permanent fact can't be derived from mutable state. The accepted, named residual: **for 15 minutes a bait-and-switch is possible.** `editPost` throws once the gate trips and treats an absent `title` as "leave it alone", so the rule holds at the API boundary, not just by hiding the field; the edit form renders the title as read-only text once locked. Full 24h title mutability and a "feed-visible EDITED marker" both stay rejected — a marker doesn't un-harvest the votes.

### Fixed

- **Mod action row no longer reflows when a reason form opens.** `.post-actions` was `align-items: center`, so expanding a `collapse`/`remove` `<details>` made the row tall and dropped the short `edit` link to the vertical middle — landing it beside the reason form and reading as though the verbs had reordered. `.post-actions` and `.mod-controls` now align on `baseline`, so `edit · collapse · remove` stay on one line and the reason form opens on its own line beneath. Applies to the comment action rows too (same classes).
- **Mod action strip no longer overflows the viewport on a phone.** Three things compounded: `.mod-controls` had no `flex-wrap`, so the verbs couldn't wrap and `ban` ran off the right edge; the reason textarea carried a hard `min-width: 200px`, which plus the confirm button pushed the panel past the screen; and `.post-title-line .post-actions` is `flex-shrink: 0`, so the row sized to max-content and simply overflowed instead of shrinking. Fixed intrinsically (no breakpoint needed): the strip wraps, `min-width: 0` lets it go narrower than max-content, `max-width: 100%` caps the row at its container despite `flex-shrink: 0`, and the textarea moved to `flex: 1 1 12rem` so it shrinks and the button wraps beneath. At ≤640px an open panel additionally takes a full-width row (`flex-basis: 100%; order: 1`) so the remaining verbs stay together on the line above rather than each wrapping onto its own line with `ban` orphaned under the form. Desktop keeps the single-line strip.
- **Anonymous `POST /draft` with an over-cap body returns a 400, not a 500.** A crafted POST can carry a title/body past the form's `maxlength`; `submitDraft` is the boundary and throws. The logged-in branch already caught that and re-rendered, but the anonymous branch let it fall through to a bare 500 (and burned an anon-draft limiter slot). It now catches and renders a 400 with `friendlyPostError` copy, writing no draft row. Found by the release `/code-review`.
- **Only one mod reason-form is open at a time.** `collapse`, `remove`, and `ban` could all sit expanded simultaneously, stacking three reason boxes across the row. They now share a `<details name="mod-<targetType>-<targetId>">` accordion group: opening one closes the others, and re-clicking the open one collapses it. Native HTML, so it holds on the no-JS path (browsers without `<details name>` fall back to independent toggles — degradation, not breakage). The group keys on the strip's *content* target rather than each action's own target, because `ban` addresses the author handle while collapse/remove address the post — keying per-action would put `ban` in a group of its own and let two forms sit open. Two posts on the same page keep independent strips.

### Tests

- `+31` (893 → 924): the public `/rss` firehose merges posts across every sub newest-first, excludes removed + collapsed, and returns a valid empty feed on a fresh instance; the home rssvp affordance + `<head>` alternate point at `/rss` when logged out and at the personal `/u/<token>/rss` when logged in; a control-char-obfuscated `javascript:` link (`[x](<java⟨TAB⟩script:…>)`) is filtered, covering TAB/LF/CR/NUL/VT/FF; a request body over the 1 MiB cap is rejected instead of buffered, while one just under it is accepted; the anonymous-draft per-IP limiter caps/isolates/resets/prunes correctly (4 unit) and the `/draft` route returns 429 past the cap with no row written (1 integration); an owner is capped at 5 active subs and a disabled sub frees a slot (2); an anonymous `/draft` with an over-`BODY_MAX` body is rejected with a 400 and writes no row (1); the edit route enforces the spam-pattern file (via an edited **title**), URLhaus (via an edited body), and the link cap (400 before the write, `edited_at` still null) — three tests that fail against the pre-fix route; external links carry `target`/`rel` while relative, fragment, and `mailto` links don't; `editPost` updates title in both the DB row and the on-disk frontmatter (asserted against the file, not the DB row a second time — the `.md` is the canonical record); a title containing `$&` / `` $` `` / `$1` lands literally in the frontmatter rather than expanding as a `String.replace` pattern; a padded stored title resubmitted unchanged doesn't read as a title change (so it can't trip the lock on a body-only edit); blank and over-`TITLE_MAX` titles throw; the title is editable just inside `TITLE_EDIT_WINDOW_MS` and locked just past it (with the old title surviving the blocked edit); votes and replies do **not** lock the title inside the window (guards against reintroducing the engagement-derived gate); body stays editable after the title locks, both when the title is resubmitted unchanged and when the field is omitted entirely; the post page renders `collapse`/`remove`/`ban` as three `<details>` sharing one `name` keyed on the content target.
...
### Fixed — health-watch cron targets the configured port

`bin/health-watch.sh` defaults to `http://localhost:8080`, but `deploy/plato.cron`'s watcher line never passed the URL — so on any deploy where plato listens on a non-default `PORT`, every 5-minute check hit a closed port, recorded `status=000` (connection refused, empty body → `version: unknown`), and emailed the operator a failure alert *every 5 minutes*. The line now passes `PLATO_URL=${PLATO_URL}` (rendered to `http://localhost:<PORT>`); left empty it still falls back to the `:8080` default, so standard deploys are unaffected. Deploy-tooling only; no app/version change.
...
Two real fixes (live-on-terribic mobile-review session) and one PRD lock crystallizing a design choice that was already implemented but not yet load-bearing in the doc.
...
- `src/content/markdown.js` — `<wbr>` between outbound anchor and `.ext-host` span
- `src/web/static/style.css` — flair-editor row stacks vertically at ≤640px; threshold row wraps; style.css cache `v=50 → v=51`
- `src/web/app.js` — cache token bump
- `docs/01-product/prd-open-web-revival.md` — §User Display rewrite + new §Permanently out entry
- `docs/01-product/prd-forum.md` — sister PRD updated to match
- `docs/02-features/plato.context.md` — 0.12.1/0.12.2 version trail entries
